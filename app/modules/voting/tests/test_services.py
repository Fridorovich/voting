...
from app.modules.voting.schemas import PollCreate, VoteCreate, ClosePollRequest
...
async def test_create_poll(db: Session):
    ...
    result = await create_poll(db, poll_data)
    assert "id" in result
...
async def test_get_poll_details(db: Session):
    ...
    poll_result = await create_poll(db, poll_data)
...
async def test_vote_in_closed_poll(db: Session, create_user):
    ...
    # Создаем опрос с указанием email создателя
    poll_data = PollCreate(
        title="Closed Poll",
        choices=["Yes", "No"],
        is_multiple_choice=False
    ...
    # Пробуем закрыть опрос
    await close_poll(db, poll_id, user_email="user@example.com")
    
    # Пробуем проголосовать в закрытом опросе
    with pytest.raises(HTTPException) as exc_info:
